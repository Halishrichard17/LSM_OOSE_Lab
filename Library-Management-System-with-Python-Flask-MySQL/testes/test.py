...
def test_register(client):
    ...
    response = client.post('/register', data={'name': 'test_user', 'email': 'goerge@webdamn.com', 'password': '123'})
...
def test_issue_book(client):
    ...
    # Expecting redirect status code since login is required
...
def test_save_author(client):
    ...
    assert response.status_code == 302  
# Assuming author addition succeeds, you might want to check for a different status code here
